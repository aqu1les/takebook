...
export default NewBook = props => {
    ...
    const previewBackCover = useMemo(
        () => (backCover ? 'data:image/jpeg;base64,' + backCover.data : null),
        [backCover],
    );
                ...
                <View style={Style.CoverContainer}>
                    {cover ? (
                        ...
                        <View
                            ...
                            onTouchEnd={handleCoverPicker}></View>
                    )}
                    {backCover ? (
                        <Image
                            source={{ uri: previewBackCover }}
                            style={Style.BackCover}
                        />
                    ) : (
                        <View
                            style={[Style.BackCover, Style.ImageHolder]}
                            onTouchEnd={handleBackCoverPicker}></View>
                    )}
                </View>
